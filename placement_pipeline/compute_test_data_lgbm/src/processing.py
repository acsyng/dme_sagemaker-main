...
# Read recipe inputs
# load the model
def test_data_lgbm_function(DKU_DST_ap_data_sector, DKU_DST_analysis_year, pipeline_runid, args, logger):
    try:
        ...
        test_data_df = pd.read_parquet(
            os.path.join(args.s3_input_trial_data_train_folder, 'test_data', DKU_DST_ap_data_sector,
                         DKU_DST_analysis_year, 'test_data.parquet'))
        if DKU_DST_ap_data_sector == "CORN_BRAZIL_SUMMER" or DKU_DST_ap_data_sector == "CORN_BRAZIL_SAFRINHA":
            test_data_df = test_data_df[test_data_df.tpp_region != 'B']
            print('test_data_df shape after filtering: ', test_data_df.shape)
        if DKU_DST_ap_data_sector == "SUNFLOWER_EAME_SUMMER":
            # test_data_df = test_data_df[test_data_df.tpp_region != '4']
            print('test_data_df shape after filtering: ', test_data_df.shape)
        if DKU_DST_ap_data_sector == "SOY_BRAZIL_SUMMER":
            test_data_df = test_data_df[(test_data_df.tpp_region != 'B') & (test_data_df.tpp_region != 'P')]
            print('test_data_df shape after filtering: ', test_data_df.shape)
        ...
        if DKU_DST_ap_data_sector == 'SOY_BRAZIL_SUMMER' or DKU_DST_ap_data_sector == 'SOY_NA_SUMMER':
            test_data_lgbm_df = create_output_soy(test_data_df, DKU_DST_ap_data_sector, trial_info_df, estimator,
            ...
            test_data_lgbm_df = create_output_cornsilage(test_data_df, trial_info_df, estimator,
                                                         allmodels_estimator_10, allmodels_estimator_50,
                                                         allmodels_estimator_90,
                                                         rfecv_mask,
                                                         DKU_DST_ap_data_sector)
        else:
            test_data_lgbm_df = create_output(test_data_df, trial_info_df, estimator,
                                              allmodels_estimator_10, allmodels_estimator_50, allmodels_estimator_90,
                                              rfecv_mask, DKU_DST_ap_data_sector)

        test_data_lgbm_df_dir_path = os.path.join('/opt/ml/processing/data/test_data_lgbm', DKU_DST_ap_data_sector,
                                                  DKU_DST_analysis_year)
        test_data_lgbm_df_data_path = os.path.join(test_data_lgbm_df_dir_path, 'test_data_lgbm.parquet')
        print('test_data_lgbm_importance_data_path: ', test_data_lgbm_df_data_path)
        isExist = os.path.exists(test_data_lgbm_df_dir_path)
        if not isExist:
            # Create a new directory because it does not exist
            os.makedirs(test_data_lgbm_df_dir_path)

        # Write recipe outputs
        test_data_lgbm_df.to_parquet(test_data_lgbm_df_data_path)

    except Exception as e:
        logger.error(e)
        error_event(DKU_DST_ap_data_sector, DKU_DST_analysis_year, pipeline_runid, str(e))
        message = f'Placement test_data_lgbm error report for {ENVIRONMENT} {pipeline_runid} {DKU_DST_ap_data_sector} {DKU_DST_analysis_year}'
...
def main():
    ...
    with open('/opt/ml/processing/input/input_variables/query_variables.json', 'r') as f:
        ...
        for input_year in years:
            file_path = os.path.join(get_s3_prefix(ENVIRONMENT), 'compute_test_data_lgbm/data/test_data_lgbm',
                                     DKU_DST_ap_data_sector, input_year, 'test_data_lgbm.parquet')
            ...
            test_data_lgbm_function(DKU_DST_ap_data_sector, input_year, pipeline_runid, args=args, logger=logger)
            print('File created')
            # print()
