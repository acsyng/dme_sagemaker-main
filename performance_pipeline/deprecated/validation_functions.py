...
def get_global_validation_vars():
...
def get_partition_from_years(years, parts):
    parts_out = []
...
def compute_weighted_trait_score(df_2yrs, merge_cols, trait_list=None,
...
def compute_advancement_rate(df_rec_valid, recommendation_methods, adv_col='was_adv', rec_col_suffix='_normRec'):
...
def compute_trait_statistics(df_rec_valid, recommendation_methods, trait_list, adv_col='was_adv',
                             score_pref='prediction_delta', rec_col_suffix='_normRec'):
...
def boolean_nan_protected_agg(df):
...
def check_advancement(df_in, current_stage, next_stage):
    return (df_in[current_stage] < df_in[next_stage]) & \
        (df_in[current_stage] < 7) & (df_in[next_stage] < 7)


def count_predictions(df_in, truth_col, pred_col):
...
def compute_confusion_matrix_stats(df_in, methods, adv_col='was_adv'):
    # useful vars
    ...
    for meth in methods:
        ...
        n_tp, n_fp, n_tn, n_fn = count_predictions(df_in, adv_col, rec_col)
        # append to output
