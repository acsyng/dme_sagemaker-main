...
class PerformanceIngestionPipeline:
    def __init__(self):
        print("start init in Performance Ingestion Pipeline pipeline!")
        self.bucket = CONFIG.get('bucket')
        self.pipeline_name = f'ap-{ENVIRONMENT}-performance-ingestion'
    ...
    def make_python_processor(self, network_config, image_uri=None, instance_type=None, instance_count=None):
        ...
        python_processor = ScriptProcessor(
            ...
            max_runtime_in_seconds=4*60*60 # set to 4 hours, convert to seconds
        )
    ...
    def set_parameters(self):
        ...
        self.param_analysis_type = ParameterString(name='analysis_type')
        self.param_input_years = ParameterString(name='input_years')
        self.param_target_pipeline_runid = ParameterString(name='target_pipeline_runid')
        self.param_is_inferring = ParameterString(name='is_inferring', default_value='0')
    ...
    def create_python_processing_step(
        ...
        if self.init_script_step:
            inputs += [ProcessingInput(
                ...
                source=self.get_destination(sub_folder='init_script_ingestion', include_pipeline_runid=True,pipeline_name='ingestion'),
        ...
        step = ProcessingStep(
            name=f'ap-{ENVIRONMENT}-compute_{name}_step',
    ...
    def compute_folder_check(self):
        self.folder_check_step = self.create_python_processing_step(
            name='folder_check',
            job_outputs=[
                ProcessingOutput(
                    output_name='output_data_ingestion',
                    source=f'{self.output_source}data_check/last_update/',
                    destination=self.get_destination(
                        sub_folder='last_update/',
        ...
        self.data_ingestion_step = self.create_python_processing_step(
            ...
            job_inputs=[
                ProcessingInput(
                    ...
                    destination='/opt/ml/processing/input/last_update'
                )],
            job_outputs=[
                ProcessingOutput(
                    output_name='output_data_ingestion',
                    source=f'{self.output_source}data_check/',
                    destination=self.get_destination(
                        sub_folder='',
                        include_pipeline_runid=False,
                        pipeline_name=''
            ...
            job_arguments=[
                '--s3_input_data_ingestion_folder', f'{self.output_source}data_check/data_ingestion/',
                '--output_load_data_ingestion_folder', f'{self.output_source}data_check/data_ingestion/'
            ]
        )


    def compute_load_data_ingestion(self):
        self.load_data_ingestion_step = self.create_python_processing_step(
            name = 'load_data_ingestion',
            job_inputs=[
                ProcessingInput(
                    input_name='input_data_ingestion',
                    source=self.get_destination(
            ...
            job_outputs=[
                ProcessingOutput(
                    output_name='output_load_data_ingestion',
                    source=f'{self.output_source}load_data_ingestion/',
                    destination=self.get_destination(
                        sub_folder='data_ingestion',
                        include_pipeline_runid=False,
                        pipeline_name=''
            ...
            job_arguments=[
                ...
                '--output_load_data_ingestion_folder', f'{self.output_source}load_data_ingestion/'
            ]
        )
    ...
    def init_script(self):
        ...
        self.query_variables = PropertyFile(
            name="do_split",
            output_name="output",
            path="/ingestion/do_split.json",
        )
        self.init_script_step = self.create_python_processing_step(
            'init_script_ingestion',
            job_outputs=[
                ProcessingOutput(
                    output_name='output',
                    source=self.output_source,
                    destination=self.get_destination(sub_folder='',include_pipeline_runid=False,pipeline_name='')
                )
            ],
            job_arguments=[
                '--ap_data_sector', self.param_ap_data_sector,
                '--analysis_type', self.param_analysis_type,
                '--input_years', self.param_input_years,
                '--is_inferring', self.param_is_inferring,
    ...
    def pipeline_preparation(self):
        #self.folder_check_step.add_depends_on([self.init_script_step])
        pipeline_split_condition = ConditionEquals(
            left=JsonGet(
                step_name=self.init_script_step.name,
                property_file=self.query_variables,
                json_path="analysis_split",
            ),
            right=True
        )
        condition_step = ConditionStep(
            name=f"ap-{ENVIRONMENT}-condition_step",
            conditions=[pipeline_split_condition],
            if_steps=[],
            else_steps=[self.data_ingestion_step],
        )
        condition_step.add_depends_on([self.init_script_step])
        #self.data_ingestion_step.add_depends_on([self.init_script_step])
        #self.trigger_performance_infer_step.add_depends_on([self.data_ingestion_step])

        self.pipeline = Pipeline(
            name=self.pipeline_name,
            parameters=[self.param_ap_data_sector,
                        self.param_analysis_type,
                        self.param_input_years,
                        self.param_is_inferring,
    ...
    def trigger_performance_infer(self):
        """
        Trigger performance infer pipeline
        """
        self.trigger_performance_infer_step = self.create_python_processing_step(
            'trigger_performance_infer',
            job_arguments=[
                '--ap_data_sector', self.param_ap_data_sector,
                '--analysis_type', self.param_analysis_type,
                '--forward_model_year', self.param_input_years,
                '--material_type', 'entry',
                '--target_pipeline_runid', self.param_target_pipeline_runid,
                '--is_inferring', self.param_is_inferring,
                '--force_refresh', self.param_force_refresh
    ...
    def create_pipeline(self):
        ...
        #self.compute_folder_check()
        self.compute_data_ingestion()
        #self.compute_load_data_ingestion()
        #self.trigger_performance_infer()
        self.pipeline_preparation()

    def get_destination(self, sub_folder='', include_pipeline_runid=True, pipeline_name='ingestion', is_spark=False):
...
if __name__ == '__main__':
    print("start main in Performance Ingestion Pipeline pipeline.")
