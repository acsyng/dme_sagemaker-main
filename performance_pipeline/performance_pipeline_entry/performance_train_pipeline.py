...
class PerformanceTrainPipeline:
    def __init__(self):
        print("start init in Performance Pipeline pipeline!")
        self.bucket = CONFIG.get('bucket')
        self.pipeline_name = f'ap-{ENVIRONMENT}-performance-train'
    ...
    def set_parameters(self):
        ...
        self.param_train_years = ParameterString(name='input_years')
        ...
        self.stages = ParameterString(name='stages', default_value='1,2,3,4,5,6')
        self.param_force_refresh = ParameterString(name='force_refresh', default_value='True')
        self.param_do_hyperparam_tuning = ParameterString(name='do_hyperparameter_tuning', default_value='False')
    ...
    def compute_preprocessing_train(self):
        job_inputs = [
            ProcessingInput(
        ...
        job_arguments = [
            '--s3_input_data_ingestion_folder', f'{self.output_source}data_ingestion/'
        ]

        self.preprocessing_train_step = self.create_python_processing_step(
            name='preprocessing_train',
            job_inputs=job_inputs,
            job_outputs=[
                # Mount the src dir with your code here.
                ProcessingOutput(
                    output_name='output_preprocessing_train',
                    source=f'{self.output_source}preprocessing_train/',
    ...
    def compute_train(self):
        # define train model step
        self.train_step = self.create_python_processing_step(
            name='train',
            job_inputs=[
                ProcessingInput( # just the training data.
                    input_name='preprocessing_train',
                    source=self.get_destination(sub_folder='',include_pipeline_runid=True),
                    destination=f'{self.output_source}preprocessing_train/'
                )
            ],
            job_outputs=[
                # Mount the src dir with your code here.
                ProcessingOutput(
                    output_name='output_train',
                    source=f'{self.output_source}train/',
                    destination=self.get_destination(sub_folder='',include_pipeline_runid=False)
                )
            ],
            job_arguments=[
                '--s3_input_training_data_folder', f'{self.output_source}preprocessing_train/'
            ]
        )


    def compute_evaluate(self):
        # define forward model step
        self.evaluate_step = self.create_python_processing_step(
            name='evaluate',
            job_inputs=[
                ProcessingInput(
                    input_name='validation_data',
                    source=self.get_destination(sub_folder=''),
                    destination=f'{self.output_source}validation/'
                ),
                ProcessingInput(
                    input_name='model_location',
                    source=self.get_destination(sub_folder='adv_mdls',include_pipeline_runid=False),
            ...
            job_outputs=[
                # Mount the src dir with your code here.
                ProcessingOutput(
                    output_name='output_evaluate',
                    source=f'{self.output_source}evaluate/',
                    destination=self.get_destination(sub_folder=''))
            ],
            job_arguments=[
                '--s3_input_validation_data_folder', f'{self.output_source}validation/',
    ...
    def init_script(self):
        ...
        self.init_script_step = self.create_python_processing_step(
            ...
            job_arguments=[
                ...
                '--train_years', self.param_train_years,
                ...
                '--stages', self.stages,
                '--do_hyperparam_tuning', self.param_do_hyperparam_tuning,
    ...
    def pipeline_preparation(self):
        ...
        self.preprocessing_train_step.add_depends_on([self.init_script_step])
        self.train_step.add_depends_on([self.preprocessing_train_step])
        self.evaluate_step.add_depends_on([self.train_step])


        self.pipeline = Pipeline(
            name=self.pipeline_name,
            parameters=[self.param_ap_data_sector,
                        self.param_train_years,
                        self.param_forward_model_year,
                        self.param_material_type,
                        self.param_target_pipeline_runid,
                        self.param_do_hyperparam_tuning,
                        self.stages,
    ...
    def create_pipeline(self):
        ...
        self.init_script()
        self.compute_preprocessing_train()
        self.compute_train()
    ...
    def get_destination(self,
        ...
        if include_pipeline_runid:
            possible_values = [protocol, S3_BUCKET, S3_DATA_PREFIX,
                                ap_data_sector, pipeline_name, forward_model_year,
