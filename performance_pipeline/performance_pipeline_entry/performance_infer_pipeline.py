...
class PerformanceInferPipeline:
    def __init__(self):
        print("start init in Performance Pipeline pipeline!")
        self.bucket = CONFIG.get('bucket')
        self.pipeline_name = f'ap-{ENVIRONMENT}-performance-infer'
    ...
    def set_parameters(self):
        ...
        self.param_forward_model_year = ParameterString(name='forward_model_year')
        self.param_material_type = ParameterString(name='material_type', default_value='entry')
        self.param_target_pipeline_runid = ParameterString(name='target_pipeline_runid')
        self.param_force_refresh = ParameterString(name='force_refresh', default_value='False')
        ...
        if job_outputs:
            ...
            outputs = [
                ...
                ProcessingOutput(
                    ...
                    destination=self.get_destination(sub_folder=name)
                )
            ]
    ...
    def compute_preprocessing_infer(self):
        self.preprocessing_infer_step = self.create_python_processing_step(
            name='preprocessing_infer',
            job_inputs=[
                ProcessingInput(
                    input_name='data_ingestion',
                    source=self.get_destination(
                        sub_folder='data_ingestion',
                        pipeline_name='',
                        include_model_year=False,
                        include_pipeline_runid=False
                    ),
                    destination=f'{self.output_source}inference_data/'
                ),
                ProcessingInput(
                    input_name='preprocessors',
                    source=self.get_destination(
                        sub_folder='preprocessors',
                        pipeline_name='train',
                        include_pipeline_runid=False
                    ),
                    destination=f'{self.output_source}preprocessors/'
                ),
                ProcessingInput(
                    input_name='model_location',
                    source=self.get_destination(
                        sub_folder='adv_mdls',
                        include_pipeline_runid=False,
                        pipeline_name='train'
                    ),
                    destination=f'{self.output_source}mdl/'
                )
            ],
            job_outputs=[
                # Mount the src dir with your code here.
                ProcessingOutput(
                    output_name='output_preprocessing_infer',
                    source=f'{self.output_source}preprocessing_infer/',
                    destination=self.get_destination(sub_folder='', include_pipeline_runid=False))
            ],
            job_arguments=[
                '--s3_input_preprocessor_folder', f'{self.output_source}preprocessors/',
                '--s3_input_model_folder', f'{self.output_source}mdl/',
                '--s3_input_data_ingestion_folder',f'{self.output_source}inference_data/'
            ]
        )

    def compute_infer(self):
        # define forward model step
        self.infer_step = self.create_python_processing_step(
            name='infer',
            job_inputs=[
                ProcessingInput(
                    input_name='inference_data',
                    source=self.get_destination(sub_folder=''), # pipeline runid
                    destination=f'{self.output_source}inference_data/'
                ),
                ProcessingInput(
                    input_name='model_location',
                    source=self.get_destination(sub_folder='adv_mdls', pipeline_name='train', include_pipeline_runid=False),
                    destination=f'{self.output_source}mdl/'
                ),
                ProcessingInput(
                    input_name='inference_results',
                    source=self.get_destination(sub_folder='inference_results',include_pipeline_runid=False),
                    destination=f'{self.output_source}inference_results/'
                )
            ],
            job_outputs=[
                # Mount the src dir with your code here.
                ProcessingOutput(
                    output_name='output_infer',
                    source=f'{self.output_source}infer/',
                    destination=self.get_destination(sub_folder='', include_pipeline_runid=False)
                )
            ],
            job_arguments=[
                '--s3_input_inference_data_folder', f'{self.output_source}inference_data/',
                '--s3_input_model_folder', f'{self.output_source}mdl/',
                '--s3_input_inference_results_folder', f'{self.output_source}inference_results/'
            ]
        )

    def compute_push_to_postgres(self):
        # define forward model step
        self.push_to_postgres_step = self.create_python_processing_step(
            name='push_to_postgres',
            job_inputs=[
                ProcessingInput(
                    input_name='inference_results',
                    source=self.get_destination(sub_folder='inference_results', include_pipeline_runid=False),
            ...
            job_outputs=[
                # Mount the src dir with your code here.
                ProcessingOutput(
                    output_name='output_push_to_postgres',
                    source=f'{self.output_source}push_to_postgres/',
                    destination=self.get_destination(sub_folder='', include_pipeline_runid=True)
                )
            ],
            job_arguments=[
                '--s3_input_inference_results_folder', f'{self.output_source}inference_results/'
    ...
    def init_script(self):
        ...
        self.init_script_step = self.create_python_processing_step(
            ...
            job_arguments=[
                ...
                '--forward_model_year', self.param_forward_model_year,
                '--material_type', self.param_material_type,
    ...
    def pipeline_preparation(self):
        #self.start_event_step.add_depends_on([self.init_script_step])
        self.preprocessing_infer_step.add_depends_on([self.init_script_step])
        self.infer_step.add_depends_on([self.preprocessing_infer_step])
        self.push_to_postgres_step.add_depends_on([self.infer_step])

        self.pipeline = Pipeline(
            name=self.pipeline_name,
            parameters=[self.param_ap_data_sector,
                        self.param_forward_model_year,
                        self.param_material_type,
    ...
    def create_event(self, status, message):
        ...
        step = ProcessingStep(name=f'ap-{ENVIRONMENT}-send_{status.lower()}_event',
                              code=f'{self.libs_uri}/performance_send_event.py',
    ...
    def create_pipeline(self):
        ...
        self.compute_preprocessing_infer()
        self.compute_infer()
        self.compute_push_to_postgres()
        self.pipeline_preparation()

    def get_destination(self,
                        sub_folder='',
                        include_pipeline_runid=True,
                        pipeline_name='infer',
                        include_model_year=True,
                        is_spark=False):
        # set output folder destination
        # automatically append data sector and forward model year to folder structure
        # want to save all models and preprocessors in separate folder from data for
        # easier access when inferring
        protocol = 's3a:/' if is_spark else 's3:/'

        # data ingestion output is not tied to the model year
        ...
        if include_pipeline_runid:
            possible_values=[protocol, S3_BUCKET, S3_DATA_PREFIX,
                            self.param_ap_data_sector, pipeline_name, forward_model_year,
...
if __name__ == '__main__':
    print("start main in performance Pipeline pipeline.")
    sp = PerformanceInferPipeline()
    sp.create_pipeline()
    print("end main in performance Pipeline pipeline.")